...
class Canvas(QLabel):
    ...
    def deleteTrajectory(self, index):        
        ...
        os.remove("point"+str(index+1)+".txt") 
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.isDrawing == 1:
            ...
            if (self.my_qdilatedimage.pixel(event.pos()) == self.my_qdilatedimage.pixel(QPoint(self.start_x * self.distance_ratio, self.start_y * self.distance_ratio)) and self.isDrawing==1) :
...
class MainWindow(QMainWindow):
    ...
    def deletecheckBox(self):
        ...
        #redefinir les noms des courbes
        #supprimer et ajouter les checkboxes
        ...
        for i in range(indice[0], self.canvas.compteur):
            print(self.numero[i]+1)
            self.listCheckBox.append("courbe"+str(self.numero[i]+1))
        #associer les noms a des cases a cocher
        j=0
        while j<indice[0]:
            self.listCheckBox[j]="courbe"+str(self.numero[j]+1)
            j+=1
